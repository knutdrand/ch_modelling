...
from ch_modelling.models.flax_models.rnn_model import Preprocess, ARModel2, MultiValueARAdder

from ch_modelling.tuned_models import ar_model_weekly_v2
model = ar_model_weekly_v2()
model.set_model(ARModel2(
    Preprocess(output_dim=2, dropout_rate=0.2),
    SimpleCell(features=4),
    SimpleCell(features=4s),
    ar_adder=MultiValueARAdder()))
#model._model.ar_adder=MultiValueARAdder()
# model.set_model(ARModel2(
#      Preprocess(output_dim=2, dropout_rate=0.2),
#      SimpleCell(features=4),
#      SimpleCell(features=4),
...
if __name__ == '__main__':
    ...
    results = evaluate_model(model, dataset, prediction_length=12, n_test_sets=41, report_filename='laos_weekly_report_2_multi.pdf',
                             weather_provider=QuickForecastFetcher)
    print(results)
